        ...
        if (steps.env.REPOSITORY_UNDER_TEST && steps.env.LIBRARY_VERSION_UNDER_TEST) {
            ...
            steps.dockerExecute(script: steps, dockerImage: 'golang:1.13', dockerOptions: '-u 0', dockerEnvVars: [
                ...
                steps.dir("jenkins-library-${steps.env.LIBRARY_VERSION_UNDER_TEST}") {
                    steps.sh "CGO_ENABLED=0 go build -tags release -ldflags \"-X github.com/HoffmannThomas/jenkins-library/cmd.GitCommit=${steps.env.LIBRARY_VERSION_UNDER_TEST}\" -o ../piper . && chmod +x ../piper && chown 1000:999 ../piper"
                }
            ...
            def fallbackUrl = 'https://github.com/HoffmannThomas/jenkins-library/releases/latest/download/piper_master'
            def piperBinUrl = (version == 'master') ? fallbackUrl : "https://github.com/HoffmannThomas/jenkins-library/releases/download/${version}/piper"
