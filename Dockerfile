...
RUN export GIT_COMMIT=$(git rev-parse HEAD) && \
    ...
    CGO_ENABLED=0 go build \
        -ldflags \
            "-X github.com/HoffmannThomas/jenkins-library/cmd.GitCommit=${GIT_COMMIT} \
            -X github.com/HoffmannThomas/jenkins-library/pkg/log.LibraryRepository=${GIT_REPOSITORY} \
            -X github.com/HoffmannThomas/jenkins-library/pkg/telemetry.LibraryRepository=${GIT_REPOSITORY}" \
        -tags release \
        -o piper
